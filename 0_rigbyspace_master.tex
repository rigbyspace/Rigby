...
\section{Discrete Replacement for Modulo}

\subsection{Motivation}
The classical expression $t \pmod N$ presupposes a quotient operation and the algebraic structure of modular arithmetic. In a strictly discrete physical universe, the concept of ``quotienting'' is an abstraction that obscures the underlying mechanism of cyclic evolution. We replace the arithmetic modulo with explicit state evolution on a finite directed graph. This provides a geometric and causal interpretation of periodicity, where time is measured by transitions between discrete states rather than by division.

\subsection{Definition of the Phase Space}

The fundamental arena for cyclic dynamics is the Phase Space. This is not a continuous circle but a finite set of discrete states.

\begin{definition}[Phase Space]
For a fixed integer $N \ge 1$, define the phase space
\[ \Phi_N := \{0, 1, 2, \dots, N-1\}. \]
This is a finite set of integer states representing the available distinct phases of a system with capacity $N$.
\end{definition}
...
\begin{figure}[hbt!]
    ...
    \begin{tikzpicture}[>=Stealth, auto, node distance=2.5cm, thick]
      ...
      \def\R{2.5} 
      \foreach \i in {0,...,7} {
        \node[draw, circle, minimum size=0.8cm, fill=white] (s\i) at ({90 - \i*360/\N}:\R) {$\i$};
      }
      \foreach \i [evaluate=\i as \j using {int(mod(\i+1, \N))}] in {0,...,7} {
        \draw[->, bend left=15] (s\i) to node[midway, font=\footnotesize] {succ} (s\j);
      }
      \draw[->, bend left=15, dashed, red] (s7) to node[midway, above, font=\footnotesize, red] {reset} (s0);
      \node[align=center, font=\bfseries] at (0,0) {Phase Space $\Phi_{N}$\\$N=8$};
    \end{tikzpicture}
    \caption{The discrete Phase Space $\Phi_N$ (here $N=8$) replacing the continuum circle. Evolution is defined by the Successor Map, forming a closed directed graph.}
    \label{fig:phase_space}
\end{figure}

\subsection{Successor-with-Reset Operator}

Movement within the phase space is governed by a deterministic operator that enforces the boundary condition.

\begin{definition}[Successor Map]
Define a function $\operatorname{succ}_N : \Phi_N \to \Phi_N$ by:
\[
\operatorname{succ}_N(s) = \begin{cases} 
s + 1, & \text{if } s < N - 1, \\
0, & \text{if } s = N - 1.
\end{cases}
\]
This function is total, deterministic, and defined entirely by integer comparison. It generates the causal flow of time within the cycle.
\end{definition}
...
Given an initial phase $s_0 \in \Phi_N$, define the phase at time $t \in \mathbb{Z}_{\ge 0}$ recursively:
\[ \phi_N(0) := s_0, \quad \phi_N(t+1) := \operatorname{succ}_N(\phi_N(t)). \]
No arithmetic reduction or division is involved; the state evolves stepwise.
\end{definition}

\subsection{Axioms for Discrete Modularity}

We establish two axioms that govern the behavior of cyclic systems.

\begin{axiom}[Finite Cyclicity]
The directed graph induced by $\operatorname{succ}_N$ is a single cycle of length $N$.
\end{axiom}

\begin{axiom}[Determinism]
For fixed $N$ and $s_0$, the phase $\phi_N(t)$ is uniquely defined for all $t$.
\end{axiom}
...
For all $t \ge 0$, $\phi_N(t + N) = \phi_N(t)$.
\end{lemma}
\begin{proof}
After $N$ applications of $\operatorname{succ}_N$, the cycle returns to its starting state by the definition of the graph structure.
\end{proof}

\begin{lemma}[Phase Uniqueness]
For $0 \le t_1 < t_2 < N$, $\phi_N(t_1) \neq \phi_N(t_2)$.
\end{lemma}

\subsection{Theorem: Modulo Elimination}

\begin{theorem}[Discrete Modulo Replacement]
Any construction that would traditionally reference $t \pmod N$ is replaced by the phase $\phi_N(t)$.
\end{theorem}
...
\subsection{Absolute Value (Eliminated)}
...
\begin{definition}[Inertial Mass and Structural Entropy]
For a state $s = (n, d) \in S_L$:
\begin{enumerate}
    \item \textbf{Inertial Mass ($m_I$)}: Defined as the raw integer value of the denominator.
    \[ m_I(s) := d \in \mathbb{Z}_{\geq 0} \]
    \item \textbf{Structural Entropy ($S$)}: Defined as the discrete rank of the inertial mass.
    \[ S(s) := \rho(d) \]
\end{enumerate}
\textit{Interpretation:} Inertia measures the vacuum operations required to lift the state (linear weight); Entropy measures the causal history required to generate it (complexity class).
\end{definition}
...
The \textbf{Lattice Coupling Constant} is the discrete analogue of the fine-structure constant. It measures the asymptotic ratio between interaction events ($\psi$) and propagation steps in a closed cycle.

\begin{definition}[Lattice Coupling Constant]
Let $\Omega_N$ be the set of all valid state transitions in a closed phase cycle of modulus $N$. Let $\Omega_\psi$ be the subset of transitions requiring a Transformative Reciprocal ($\psi$) operation. The Lattice Coupling Constant is defined by the discrete stabilization of the ratio:
\[
\alpha_{\text{lat}} := (\text{count}(\Omega_\psi), \text{count}(\Omega_N)) \in S_L
\]
where $\alpha_{\text{lat}}$ is the unique Explicit Rational Pair such that for all $N \ge N_{\text{crit}}$, the count ratio remains invariant. The constant is a property of the finite saturated lattice; no analytic limit is invoked.
\end{definition}

\begin{theorem}[Discrete Fine-Structure Analogue]
Assume that in a physically relevant regime, the Lattice Coupling Constant $\alpha_{\text{lat}}$ stabilizes. Then:
\begin{enumerate}
    \item $\alpha_{\text{lat}}$ is a purely integer object $(n^*, d^*)$.
    \item All references to coupling strength can be expressed in terms of $\alpha_{\text{lat}}$ and its rank-based properties without invoking real numbers.
\end{enumerate}
\end{theorem}
...
\subsection{Algorithm 17.1 (Precession Calculation)}
\begin{enumerate}
...
\begin{description}
    ...
    \item[Structural Entropy ($S$)] A measure of the causal history of a state, defined as the discrete rank of its inertial mass (denominator): $S(s) = \rho(d)$.
